...
# =========================================================
# 1. BEZPIECZNE POBIERANIE SEKRETÓW
# =========================================================

def get_secrets():
    """Pobiera sekrety w sposób bezpieczny, nie przerywając działania aplikacji."""
    secrets = {
        "GEMINI_KEY": None,
        "FIREBASE_SERVICE_ACCOUNT": None
    }
    try:
        secrets["GEMINI_KEY"] = st.secrets.get("GEMINI_KEY")
        secrets["FIREBASE_SERVICE_ACCOUNT"] = st.secrets.get("FIREBASE_SERVICE_ACCOUNT")
    except Exception:
        pass
    return secrets
...
@st.cache_resource
def init_db():
    if not SECRETS["FIREBASE_SERVICE_ACCOUNT"]:
        return None
    try:
        creds_info = json.loads(SECRETS["FIREBASE_SERVICE_ACCOUNT"])
...
st.markdown('<div style="text-align:center; margin-top:40px;"><h1 style="font-weight:100; letter-spacing:1rem;">SZEPT</h1></div>', unsafe_allow_html=True)

# Sprawdzenie konfiguracji kluczy
if not API_KEY or not SECRETS["FIREBASE_SERVICE_ACCOUNT"]:
    st.markdown(f"""
    <div class="config-guide">
        <h3 style="color:#F8FAFC; font-weight:200;">Wymagana Konfiguracja</h3>
        <p style="color:#94A3B8; font-size:0.9rem;">Aplikacja nie odnalazła Twoich kluczy w <b>.streamlit/secrets.toml</b>.</p>
        <p style="color:#94A3B8; font-size:0.8rem;">Upewnij się, że plik wygląda tak:</p>
        <pre style="background:#000; color:#50fa7b; padding:10px;">
GEMINI_KEY = "Twoj_Klucz_API"
FIREBASE_SERVICE_ACCOUNT = '''
{{
  "project_id": "szept-c417f",
  ... (reszta JSON)
}}
'''</pre>
    </div>
...
if not st.session_state.finished:
    ...
    if "current_aura" not in st.session_state:
        with st.spinner("Architekt dekonstruuje Twój szept..."):
            sys = "Jesteś poetą. JSON: {\"aura\": \"nazwa\", \"quote\": \"sentencja\"}"
